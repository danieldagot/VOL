...
func TestExamplesRemainExecutable(t *testing.T) {
	tests := []struct {
		...
		{file: filepath.Join("features", "result.vol"), output: "ok: 7\nerr: missing\n"},
		{file: filepath.Join("features", "result_helpers.vol"), output: "10/2 = 5\nerror: divide by zero\n"},
		{file: filepath.Join("features", "option_result.vol"), output: "found: VOL\nempty name\npositive: 3\nerror: negative\n"},
		{file: filepath.Join("features", "errors.vol"), output: "parsed: 85\nparse failed: score out of range\ngrade: pass\ngrade failed: score out of range\naverage: 80\naverage failed: empty list\n"},
